...
import {FAB, Modal, Portal, Text, Button, Provider, TextInput,ActivityIndicator, Colors } from 'react-native-paper';
...
const ManageRent = () => {
  ...
  const renderItem = ({item}) => <Item key={item._id} item={item} status={rentStatus.status} handleChangeStatus={handleChangeStatus}/>;
